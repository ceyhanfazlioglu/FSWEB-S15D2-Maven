...
public class StringSet {
    ...
    public static Set<String> findUniqueWords() {
        ...
        // Uzun tire (—) işaretini kelimelerin bitişmesini önlemek için normal boşluk yapıyoruz
        String textWithoutDash = text.replace("—", " ");

        // . , ! ? " karakterlerini temizliyoruz (Büyük/küçük harfe dokunmuyoruz)
        String cleanedText = textWithoutDash.replaceAll("[.,!?\"]", "");

        // Boşluklara göre kelimeleri ayırıyoruz
        String[] words = cleanedText.split("\\s+");

        // Doğal sıralama (Alfabetik) ve benzersizlik için TreeSet
